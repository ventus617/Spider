...
int Analyse_url(url_t* url);//对原始地址进行分析，提取关键要素
...
int Analyze_html(url_t* url,Container* wct,Container* yct);//网页源码解析,分析标题，描述，新链接，持久化，新链接循环处理
Container* Create_Container(int num);//创建出队和入队队列
int Save_result(const char* title,const char* description);//用html格式存储在表格中
